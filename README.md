...
3. 写文章时选择文章类型：日报、周报或普通文章。日报必须填写“日报归属日期”，周报必须填写 ISO 周次；slug 是文章的唯一标识，建议使用 `daily-YYYY-MM-DD-说明` 或 `weekly-YYYY-Www`，但 Election 不依赖 slug 判断分类。

## 6. Election 日报与周报规范

文章发布时会同时写入 `config.json`。GitHub Actions 会在每次推送后重新扫描所有文章配置并确定性重建 `public/blogs/index.json`，成员不需要也不应手动编辑索引。

| 文章类型 | 必填字段 | Election 用途 |
| --- | --- | --- |
| 日报 `daily` | `reportDate: YYYY-MM-DD` | 按上海 ISO 周拉取；同日多篇和补写往期均可 |
| 周报 `weekly` | `week: YYYY-Www` | 标记与打开该周已发布的个人周报 |
| 普通文章 `article` | 无 | 不参与日报、周报汇总 |

索引校验命令：

```bash
pnpm blog:index:check
```

Actions 仅使用仓库自带的 `GITHUB_TOKEN` 与 `contents: write` 权限，不需要新增 Secret。若发现日报日期、周次或文章路径不合法，工作流会失败，防止坏数据进入索引。

## 从现在开始, 你也拥有一个自己的小网站需要自己去维护了, 加油吧, lec未来的诸位大佬们 (ゝ∀･) !!!
